...
const NO_FILES_WARNING =
  ...
  `    # ...\n` +
  `    - uses: 'step-security/upload-cloud-storage@v3'\n` +
  `\n` +
...
async function validateSubscription(): Promise<void> {
  ...
  const upstream = 'google-github-actions/upload-cloud-storage'
...
export async function run(): Promise<void> {
  try {
    ...
    const destinations = Client.computeDestinations({
      ...
      // its own name: step-security/upload-cloud-storage#259.
    ...
    const msg = errorMessage(err);
    core.setFailed(`step-security/upload-cloud-storage failed with: ${msg}`);
  }
}
